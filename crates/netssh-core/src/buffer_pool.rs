...
impl BufferPool {
    ...
    pub fn new(max_size: usize, reuse_threshold: usize) -> Self {
        debug!("Creating new buffer pool with max_size={}, reuse_threshold={}", max_size, reuse_threshold);
        Self {
    ...
    fn return_buffer(&self, mut buffer: Vec<u8>) {
        ...
        if buffer.capacity() <= self.reuse_threshold {
            ...
            if pool.len() < self.max_size {
                trace!("Returning buffer with capacity {} to pool", buffer.capacity());
...
impl BorrowedBuffer {
    ...
    pub fn read_from<R: std::io::Read>(&mut self, reader: &mut R, max_len: usize) -> std::io::Result<usize> {
        // Ensure we have enough capacity
        let current_len = self.buffer.len();
        let needed_capacity = current_len + max_len;
        
        if self.buffer.capacity() < needed_capacity {
            self.buffer.reserve(needed_capacity - self.buffer.capacity());
        }
