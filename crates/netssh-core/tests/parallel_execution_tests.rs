...
impl MockDeviceFactory {
    fn create_device(config: &DeviceConfig, commands: HashMap<String, Result<String, NetsshError>>, connect_result: Result<(), NetsshError>, delay_ms: Option<u64>) -> Box<dyn NetworkDeviceConnection + Send> {
...
async fn test_parallel_execute_command_success() {
    ...
    for device in &devices {
        ...
        commands.insert(command.clone(), Ok(format!("Version info for {}", device.host)));
...
async fn test_parallel_execute_commands_on_all() {
    ...
    // Execute commands
    let results = manager.execute_commands_on_all(devices, commands.clone()).await.unwrap();
...
async fn test_failure_strategy_stop_device() {
    ...
    commands_map.insert(commands[1].clone(), Err(NetsshError::CommandError("Command failed".to_string())));
...
async fn test_failure_strategy_stop_all() {
    ...
    device1_commands.insert(commands[1].clone(), Err(NetsshError::CommandError("Command failed".to_string())));
    ...
    device2_commands.insert(commands[1].clone(), Ok("Interfaces info device2".to_string()));
    ...
    let has_failed_command = device1_results.iter()
...
async fn test_command_timeout() {
    ...
    // Execute command
    let device_command_map: HashMap<DeviceConfig, Vec<String>> = HashMap::from([
        (device, vec![command.clone()])
    ]);
    ...
    assert_eq!(device1_results.len(), 1);
    assert_eq!(device1_results[0].command, command);
    assert!(device1_results[0].status == CommandStatus::Timeout || device1_results[0].status == CommandStatus::Failed);
}
