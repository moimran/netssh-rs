use netssh_core::semaphore::{SemaphoreError, TimeoutSemaphore};
...
fn test_semaphore_multi_thread() {
    ...
    for _ in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            // Acquire a permit
            if let Ok(_permit) = sem_clone.acquire_timeout(Some(Duration::from_millis(500))) {
...
fn test_semaphore_close() {
    ...
    assert!(matches!(
        sem.acquire_timeout(Some(Duration::from_millis(10))),
        Err(SemaphoreError::Closed)
    ));
...
fn test_semaphore_add_remove_permits() {
    ...
    // Remove 2 permits while holding 2 permits
    ...
    // After releasing permits, we should have 3 available (max_permits)
    assert_eq!(sem.available_permits().unwrap(), 3);
    assert_eq!(sem.max_permits().unwrap(), 3);
}
