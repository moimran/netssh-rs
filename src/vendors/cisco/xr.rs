...
impl CiscoXrSsh {
    ...
    pub fn clear_buffer(
        ...
        for _ in 0..10 {
            ...
            let data = match self.connection.read_channel() {
                Ok(data) => {
                    debug!(target: "CiscoXrSsh::clear_buffer", "Read {} bytes from channel: {:?}", data.len(), data);
                    data
                }
                Err(e) => {
                    warn!(target: "CiscoXrSsh::clear_buffer", "Error reading channel: {}", e);
                    break;
                }
            };
            ...
            debug!(target: "CiscoXrSsh::clear_buffer", "After stripping ANSI codes: {:?}", data);
            ...
            debug!(target: "CiscoXrSsh::clear_buffer", "Clear buffer detects data in the channel");
            if backoff {
                sleep_time *= 2.0;
        ...
        debug!(target: "CiscoXrSsh::clear_buffer", "Buffer cleared, accumulated {} bytes", output.len());
        Ok(output)
    }
    ...
    pub fn close(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::close", "Closing connection to device");

        // Try to exit config mode if we're in it
        debug!(target: "CiscoXrSsh::close", "is it in config mode {}", self.in_config_mode);
        ...
        if let Some(channel) = self.connection.channel.as_mut() {
            debug!(target: "CiscoXrSsh::close", "Closing SSH channel");
        ...
        debug!(target: "CiscoXrSsh::close", "Connection closed successfully");
        Ok(())
    }
}
...
impl CiscoXrSsh {
    ...
    pub fn connect(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::connect", "Connecting to {}@{}", self.config.username, self.config.host);
    ...
    pub fn check_enable_mode(&mut self) -> Result<bool, NetsshError> {
        debug!(target: "CiscoXrSsh::check_enable_mode", "Checking if device is in enable mode");
        ...
        let output = match self.connection.read_channel() {
            Ok(out) => out,
            Err(e) => {
                warn!(target: "CiscoXrSsh::check_enable_mode", "Error reading response: {}", e);
        ...
        debug!(target: "CiscoXrSsh::check_enable_mode", "Device is in enable mode (#): {}", is_enable);
        Ok(is_enable)
    }

    pub fn enable(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::enable", "Entering enable mode");

        // Check if already in enable mode
        if self.check_enable_mode()? {
            debug!(target: "CiscoXrSsh::enable", "Already in enable mode");
            return Ok(());
        }
        ...
        if let Some(secret) = &self.config.secret {
            debug!(target: "CiscoXrSsh::enable", "Waiting for password prompt");
            ...
            if PASSWORD_PATTERN.is_match(&output) {
                debug!(target: "CiscoXrSsh::enable", "Sending enable password");
                self.connection.write_channel(&format!("{}\n", secret))?;
            } else {
                warn!(target: "CiscoXrSsh::enable", "Password prompt not found in output: {}", output);
        ...
        if !output.trim().ends_with("#") {
            warn!(target: "CiscoXrSsh::enable", "Enable prompt not found");
            return Err(NetsshError::CommandError(
        ...
        debug!(target: "CiscoXrSsh::enable", "Successfully entered enable mode");

        Ok(())
    }

    pub fn exit_enable_mode(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::exit_enable_mode", "Exiting enable mode");

        // Check if already not in enable mode
        if !self.check_enable_mode()? {
            debug!(target: "CiscoXrSsh::exit_enable_mode", "Already not in enable mode");
            return Ok(());
        }
        ...
        if !output.trim().ends_with(">") {
            warn!(target: "CiscoXrSsh::exit_enable_mode", "User prompt not found after disable command");
            return Err(NetsshError::CommandError(
        ...
        self.in_enable_mode = false;
        debug!(target: "CiscoXrSsh::exit_enable_mode", "Successfully exited enable mode");
...
impl CiscoDeviceConnection for CiscoXrSsh {
    fn session_preparation(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::session_preparation", "Preparing session");
    ...
    fn set_terminal_width(&mut self, width: u32) -> Result<(), NetsshError> {
        ...
        let output = match self.connection.read_channel() {
            Ok(out) => out,
            Err(e) => {
                warn!(target: "CiscoXrSsh::set_terminal_width", "Error reading response: {}", e);
        ...
        if output.contains("Invalid") || output.contains("Error") {
            warn!(target: "CiscoXrSsh::set_terminal_width", "Error setting terminal width: {}", output);
            // Continue anyway, don't fail the connection for this
        } else {
            debug!(target: "CiscoXrSsh::set_terminal_width", "Terminal width command sent successfully");
        }
    ...
    fn disable_paging(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::disable_paging", "Disabling paging");
        ...
        let output = match self.connection.read_channel() {
            Ok(out) => out,
            Err(e) => {
                warn!(target: "CiscoXrSsh::disable_paging", "Error reading response: {}", e);
                // Continue anyway, don't fail the connection for this
                String::new()
            }
        };
        ...
        if output.contains("Invalid") || output.contains("Error") {
            warn!(target: "CiscoXrSsh::disable_paging", "Error disabling paging: {}", output);
            // Continue anyway, don't fail the connection for this
        } else {
            debug!(target: "CiscoXrSsh::disable_paging", "Paging disabled successfully");
        }
    ...
    fn set_base_prompt(&mut self) -> Result<String, NetsshError> {
        debug!(target: "CiscoXrSsh::set_base_prompt", "Setting base prompt");
        ...
        let output = match self.connection.read_until_pattern(pattern) {
            Ok(out) => out,
            Err(e) => {
                warn!(target: "CiscoXrSsh::set_base_prompt", "Error reading response: {}", e);
                // Use a default prompt if we can't read the actual one
                self.prompt = "RP/0/RP0/CPU0".to_string();
            ...
            .last()
        {
            ...
            debug!(target: "CiscoXrSsh::set_base_prompt", "Base prompt set to: {}", self.prompt);

            // Also set the base_prompt in the BaseConnection
            self.connection.base_prompt = Some(self.prompt.clone());
            debug!(target: "CiscoXrSsh::set_base_prompt", "Set base_prompt in BaseConnection to: {}", self.prompt);

            // Set the prompt in the SSHChannel
            self.connection.channel.set_base_prompt(&self.prompt);
            debug!(target: "CiscoXrSsh::set_base_prompt", "Set base_prompt in SSHChannel");
        } else {
            // If we can't find a prompt, use a default
            warn!(target: "CiscoXrSsh::set_base_prompt", "Could not find prompt in output: {}", output);
            self.prompt = "RP/0/RP0/CPU0".to_string();
    ...
    fn check_config_mode(&mut self) -> Result<bool, NetsshError> {
        debug!(target: "CiscoXrSsh::check_config_mode", "Checking if device is in config mode");
        ...
        let output = match self.connection.read_channel() {
            Ok(out) => out,
            Err(e) => {
                warn!(target: "CiscoXrSsh::check_config_mode", "Error reading response: {}", e);
        ...
        debug!(target: "CiscoXrSsh::check_config_mode", "Device is in config mode: {}", is_config);
        Ok(is_config)
    }

    fn config_mode(&mut self, config_command: Option<&str>) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::config_mode", "Entering config mode");

        // Check if already in config mode
        if self.check_config_mode()? {
            debug!(target: "CiscoXrSsh::config_mode", "Already in config mode");
            return Ok(());
        }

        // Ensure we're in enable mode first
        if !self.check_enable_mode()? {
            debug!(target: "CiscoXrSsh::config_mode", "Not in enable mode, entering enable mode first");
            self.enable()?;
        }
        ...
        if !output.contains("(config)#") {
            warn!(target: "CiscoXrSsh::config_mode", "Config prompt not found after config command: {}", output);
            return Err(NetsshError::CommandError(
        ...
        debug!(target: "CiscoXrSsh::config_mode", "Successfully entered config mode");

        Ok(())
    }

    fn exit_config_mode(&mut self, exit_command: Option<&str>) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::exit_config_mode", "Exiting config mode");

        // Check if already not in config mode
        if !self.check_config_mode()? {
            debug!(target: "CiscoXrSsh::exit_config_mode", "Already not in config mode");
            return Ok(());
        }
        ...
        if !output.trim().ends_with("#") || output.contains("(config)") {
            warn!(target: "CiscoXrSsh::exit_config_mode", "Enable prompt not found after exit command: {}", output);
            return Err(NetsshError::CommandError(
        ...
        self.in_config_mode = false;
        debug!(target: "CiscoXrSsh::exit_config_mode", "Successfully exited config mode");

        Ok(())
    }

    fn save_config(&mut self) -> Result<(), NetsshError> {
        debug!(target: "CiscoXrSsh::save_config", "Saving configuration");
        ...
        if output.contains("Error") {
            warn!(target: "CiscoXrSsh::save_config", "Error saving configuration: {}", output);
    ...
    fn send_command(&mut self, command: &str) -> Result<String, NetsshError> {
        debug!(target: "CiscoXrSsh::send_command", "Sending command: {}", command);
        ...
        debug!(target: "CiscoXrSsh::send_command", "Command output received, length: {}", result.len());
        Ok(result)
    }
}
